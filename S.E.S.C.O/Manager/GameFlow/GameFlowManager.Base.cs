...
namespace DevelopKit.BasicTemplate
{
    ...
    public partial class GameFlowManager : SingletonMonoBehaviour<GameFlowManager>
    {
        ...
        public static Action<float> UpdateSpeedChanged;
        ...
        private void Update()
        {
            ...
            for (int i = _updateHandlers.Count - 1; i >= 0; i--)
            {
                _updateHandlers[i].UpdateCallback.Invoke(Time.deltaTime * _updateSpeed);
            }
        }

        private List<PrioritizedUpdateHandler> _fixedUpdateHandlers;
        private bool isDirtyFlagOfFixedUpdateHandler;
        private void FixedUpdate()
        {
            ...
            if (isDirtyFlagOfFixedUpdateHandler)
            {
                _fixedUpdateHandlers.Sort((a, b) => b.Priority - a.Priority);
                isDirtyFlagOfFixedUpdateHandler = false;
            }

            for (int i = _fixedUpdateHandlers.Count - 1; i >= 0; i--)
            {
                _fixedUpdateHandlers[i].UpdateCallback.Invoke(Time.deltaTime * _updateSpeed);
            }
        }
        #endregion
