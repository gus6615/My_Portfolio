...
public class MapData : MonoBehaviour
{
    ...
    public void DeleteObject(Vector3Int point)
    {
        ...
        for (int i = 0; i < hit.Length; i++)
        {
            ...
            {
                EventBlock eventBlock = hit[i].transform.GetComponent<EventBlock>();
    ...
    public void LoopJemLightFade(Color color)
    {
        foreach (var floor in floorDatas)
        {
            foreach (var chunk in floor.TileChunks[10])
            {
                chunk.Tilemap.color = color;
            }
        }
    }
